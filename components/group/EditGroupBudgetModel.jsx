...
import { closeGroupAddBudget } from '@/store/slices/uiSlice'
import { fetchGroupBudgetSummary } from '@/store/slices/group/groupBudgetSummarySlice'
import { ArrowLeft } from 'lucide-react'

const EditGroupBudgetModel = ({ groupId }) => {
  const dispatch = useDispatch()
  const open = useSelector((state) => state.ui.isAddGroupBudgetOpen)
  const budgetData = useSelector((state) => state.groupBudget.data)
  ...
  const handleClose = () => dispatch(closeGroupAddBudget())
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`/api/groups/${groupId}/update-budget`, { budget })
      if (res.data.success) {
      ...
      console.error('[GROUP_BUDGET_UPDATE_ERROR]', err)
      ...
      >
        <form onSubmit={handleSubmit}>
         <SheetHeader className='flex flex-row items-center gap-3  px-0 pt-0 pb-2 border-b'>
            ...
            <SheetTitle className='text-sm text-neutral-500 font-bold'>Update Group Monthly Budget</SheetTitle>
          </SheetHeader>
...
export default EditGroupBudgetModel
